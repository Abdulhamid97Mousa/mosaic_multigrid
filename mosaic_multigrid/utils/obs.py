"""Numba JIT-compiled observation generation for the MOSAIC multigrid environment.

All functions are decorated with ``@nb.njit(cache=True)`` for near-C performance
on the hot observation-generation path. Enum values are extracted to plain
``int`` constants because Numba cannot access Python enum attributes inside
...
def gen_obs_grid_encoding(
    ...
    """
    Generate encoding for the sub-grid observed by an agent (with visibility).
...
def gen_obs_grid_vis_mask(
    ...
    mask : ndarray[int] of shape (num_agents, view_size, view_size)
        Visibility mask for each agent.
    """
    obs_grid = gen_obs_grid(grid_state, agent_state, agent_view_size)
...
def gen_obs_grid(
    ...
    # Get grid encoding with agents inserted
    if num_agents > 1:
    ...
    # Get top-left corner of each agent's observation window
    top_left = get_view_exts(agent_dir, agent_pos, agent_view_size)
    topX, topY = top_left[:, 0], top_left[:, 1]

    # Populate observation grids with rotation
    num_left_rotations = (agent_dir + 1) % 4
    obs_grid = np.empty(
        (num_agents, obs_width, obs_height, ENCODE_DIM), dtype=np.int_)
    for agent in range(num_agents):
        for i in range(obs_width):
            for j in range(obs_height):
                ...
                # Copy grid cell or wall (for out-of-bounds)
                if (0 <= x < grid_encoding.shape[0]
                        and 0 <= y < grid_encoding.shape[1]):
                    obs_grid[agent, i_rot, j_rot] = grid_encoding[x, y]
                else:
                    obs_grid[agent, i_rot, j_rot] = WALL_ENCODING

    # Each agent sees what it's carrying at its own position
...
def get_see_behind_mask(
    ...
    Returns
    -------
    see_behind_mask : ndarray[bool] of shape (num_agents, width, height)
        Boolean see-through mask.
    """
    ...
    for agent in range(num_agents):
        for i in range(width):
            for j in range(height):
                see_behind_mask[agent, i, j] = see_behind(
                    grid_array[agent, i, j])
...
def get_vis_mask(
    ...
    """
    Generate a boolean mask indicating which grid locations are visible.

    Uses a two-pass (forward + backward) ray-casting sweep from the agent's
    position outward.
...
def get_view_exts(
    ...
    """
    Get the top-left corner of each agent's observable view rectangle.
    ...
    top_left : ndarray[int] of shape (num_agents, 2)
        The (x, y) coordinates of each agent's view top-left corner.
    """
    ...
    top_left[agent_dir == RIGHT, 1] = (
        agent_y[agent_dir == RIGHT] - agent_view_size // 2)

    # Facing down
    top_left[agent_dir == DOWN, 0] = (
        agent_x[agent_dir == DOWN] - agent_view_size // 2)
    top_left[agent_dir == DOWN, 1] = agent_y[agent_dir == DOWN]

    # Facing left
    top_left[agent_dir == LEFT, 0] = (
        agent_x[agent_dir == LEFT] - agent_view_size + 1)
    top_left[agent_dir == LEFT, 1] = (
        agent_y[agent_dir == LEFT] - agent_view_size // 2)

    # Facing up
    top_left[agent_dir == UP, 0] = (
        agent_x[agent_dir == UP] - agent_view_size // 2)
    top_left[agent_dir == UP, 1] = (
        agent_y[agent_dir == UP] - agent_view_size + 1)
