...
Enhanced variant upgrades this to **teleport passing** -- the ball
...
class SoccerGameEnhancedEnv(SoccerGameEnv):
    """Enhanced Soccer game with teleport passing, ball respawn, termination,
        ...
        if fwd_obj is not None and fwd_obj.type.value == 'objgoal':
            ...
            if fwd_obj.target_type == ball.type.value:
                ...
                if ball.index in (0, fwd_obj.index):
                    ...
                    # Respawn ball at random location
                    new_ball = Ball(
                        color=Color.red,  # Wildcard ball always red
                        index=0,
                    )
...
class SoccerGame4HEnhancedEnv16x11N2(SoccerGameEnhancedEnv):
    """Enhanced 2v2 Soccer with FIFA aspect ratio (16×11 total, 14×9 playable).
