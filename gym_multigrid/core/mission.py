"""Mission and MissionSpace classes for agent task specification."""
...
class Mission(np.ndarray):
    ...
    A Mission wraps a string description with an optional discrete index
    so it can be used inside gymnasium observation spaces.
    """
    ...
    def __new__(cls, string: str, index: Iterable[int] | None = None):
        ...
        string : str
            Mission string.
        index : Iterable[int] or None
            Index of mission string in the parent :class:`MissionSpace`.
        """
    ...
    def __array_finalize__(self, mission) -> None:
        if mission is None:
            return
...
class MissionSpace(spaces.MultiDiscrete):
    """
    Space over agent missions, built on top of gymnasium MultiDiscrete.
    ...
    def __init__(
        ...
        ordered_placeholders : Sequence[Sequence[str]]
            Sequence of argument groups, ordered by placement in ``mission_func()``.
    ...
    def get(self, idx: Iterable[int]) -> Mission:
        """
        Get the mission corresponding to the given index.
